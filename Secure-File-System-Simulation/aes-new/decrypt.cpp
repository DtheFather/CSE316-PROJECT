 ...
 */
void Round_decrypt(unsigned char * state, unsigned char * key) {
...
void AESDecrypt(unsigned char * encryptedMessage, unsigned char * expandedKey, unsigned char * decryptedMessage)
{
	...
	for (int i = 8; i >= 0; i--) {
		Round_decrypt(state, expandedKey + (16 * (i + 1)));
	}
...
string decryptString(string msgstr,string keystr){
	...
	for (int i = 0; i < n; i++) {
		...
		//original_msg=original_msg+encryptedMessage[i];
	}
	...
	//cout << "****Decrypted msg in hex****" << endl;
	...
	for (int i = 0; i < messageLen; i++) {
		...
		original_msg=original_msg+(char)decryptedMessage[i];
	}
